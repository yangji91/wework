...
public class UserManager {
    ...
    public UserManager() {
        HandlerThread groupManager = new HandlerThread("UserManager");
        groupManager.start();
        handler=new UserHandler(groupManager.getLooper());
    }
...
//        Object info = RobotHelpers.callMethod(conversion, "requestInfo");
//        long conversationId =   RobotHelpers.getLongField(info, "id");
//        //先缓存
...
//    }
    ...
    private   class UserHandler extends Handler {
        public UserHandler(Looper groupManager) {
            super(groupManager);
        }
...
//    }
    ...
    public ConvMember[] getGroupMembers(ClassLoader classLoader, long conversationId) {
        ...
        if (convMembers!=null) {
            ConvMember[] rMemList = new ConvMember[convMembers.size()];
