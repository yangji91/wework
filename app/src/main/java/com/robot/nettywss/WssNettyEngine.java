...
public class WssNettyEngine {
    ...
    private Thread rQueue = new Thread() {
        ...
        public void run() {
            while (isRunning) {
                if (mHeartBeatTime != 0 && (Math.abs(mHeartBeatTime - System.currentTimeMillis()) >= MConfiger.HEART_BEAT_STOP_INTERVAL_3Minutes)) {
                    MyLog.debug(TAG, "[sendMsg]" + "超过 3分钟没有服务器数据，重新连接...", true);
                    StatsHelper.event("msgReport", "connect", "超过 3钟没有服务器数据，重新连接", "heartBeatTime" + mHeartBeatTime);
