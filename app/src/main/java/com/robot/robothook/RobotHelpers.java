...
import com.robot.robothook.utils.ClassUtils;
import com.robot.robothook.utils.MemberUtils;
...
public class RobotHelpers {
    static RobotHook robotHook;
    static boolean isFm = true;
    ...
    static {
        try {
            ...
            if (helpers != null && Bridge != null)
                setHookEnvironment(new RobotXposedHook());
        } catch (ClassNotFoundException e) {
            setHookEnvironment(new RobotFMHook());
        }

    }

    public static void setHookEnvironment(RobotHook robotHook) {
        if (robotHook != null) {
            RobotHelpers.robotHook = robotHook;
            isFm = false;
            if (robotHook instanceof RobotFMHook) {
                log(" 现在 dzoneHook环境");
                isFm = true;
            } else if (robotHook instanceof RobotXposedHook) {
                log(" 现在 xposedHook环境");
            }
        }
    }
    ...
    private RobotHelpers() {
    }


    public static void log(String s) {
        Log.d("RobotHelpers", s);
    }
    ...
    public static void log(Throwable e) {
        if (e != null) {
            ...
            for (StackTraceElement stackTraceElement : stackTraceElements) {
                Log.e("RobotHelpers", stackTraceElement.getClassName() + " Line:" + stackTraceElement.getLineNumber() + " " + stackTraceElement.getMethodName());
            }
        }
    ...
    public static void hookMethod(Member m, RobotMethodHook callback) {
        robotHook.hookMethod(m, callback);
    }


    @SuppressWarnings("UnusedReturnValue")
    public static void hookAllConstructors(Class<?> hookClass, RobotMethodHook callback) {
        robotHook.hookAllConstructors(hookClass, callback);
    ...
    @SuppressWarnings("UnusedReturnValue")
    public static void hookAllMethods(Class<?> hookClass, String methodName, RobotMethodHook callback) {
    ...
    public static Class<?> findClass(String className, ClassLoader classLoader) {
        ...
        try {
            ...
            e.printStackTrace();
            String fullMethodName = className + '#' + "未匹配到 " + e.getMessage();
            StatsHelper.event("CrashHandler", "uncaughtException", "carshInfo " + fullMethodName, "time " + StrUtils.getTimeDetailStr());
            Log.e("uncaughtException", fullMethodName);
        }
        return null;
    }
    ...
    /*public static void hookAllConstructors(Class<?> var0, XC_MethodHook var1) {
        if (isFm){
            RobotHelpers.hookAllConstructors(var0, new MethodHook() {
                @Override
                protected void beforeHookedMethod(MethodParam methodParam) throws Throwable {
                    super.beforeHookedMethod(methodParam);

                    var1.beforeHookedMethod(new RobotMethodParam(methodParam));
                }

                @Override
                protected void afterHookedMethod(MethodParam methodParam) throws Throwable {
                    var1.afterHookedMethod(new RobotMethodParam(methodParam));
                }
            });

        }else {
            if (mXposedBridge!=null){
    ...
    public static Field findField(Class<?> clazz, String fieldName) {
        String fullFieldName = clazz.getName() + "未匹配#" + fieldName;
        ...
        try {
            ...
            StatsHelper.event("CrashHandler", "uncaughtException", "carshInfo " + fullFieldName+ e, "time " + StrUtils.getTimeDetailStr() );
            throw new NoSuchFieldError(fullFieldName);
        }
    }
    ...
    public static void findAndHookMethod(Class<?> clazz, String methodName, Object... parameterTypesAndCallback) {
        if (parameterTypesAndCallback.length == 0 || !(parameterTypesAndCallback[parameterTypesAndCallback.length - 1] instanceof RobotMethodHook))
            throw new IllegalArgumentException("no callback defined");

        RobotMethodHook callback = (RobotMethodHook) parameterTypesAndCallback[parameterTypesAndCallback.length - 1];
    ...
    public static Method findMethodExact(Class<?> clazz, String methodName, Class<?>... parameterTypes) {
        String fullMethodName = clazz.getName() + '#' + methodName + getParametersString(parameterTypes) + "#未匹配到exact";
    ...
    public static Method findMethodBestMatch(Class<?> clazz, String methodName, Class<?>... parameterTypes) {
        String fullMethodName = clazz.getName() + '#' + methodName + getParametersString(parameterTypes) + "未匹配到";
        ...
        if (bestMatch != null) {
            ...
            methodCache.put(fullMethodName, null);
            StatsHelper.event("CrashHandler", "uncaughtException", "carshInfo " + fullMethodName+ e, "time " + StrUtils.getTimeDetailStr());
            throw e;
        }
    }
    ...
    private static Class<?>[] getParameterClasses(ClassLoader classLoader, Object[] parameterTypesAndCallback) {
        ...
        for (int i = parameterTypesAndCallback.length - 1; i >= 0; i--) {
            ...
            if (type instanceof RobotMethodHook)
                continue;
