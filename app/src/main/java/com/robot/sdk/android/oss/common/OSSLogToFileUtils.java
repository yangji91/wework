package com.robot.sdk.android.oss.common;
...
public class OSSLogToFileUtils {
    ...
    public void deleteLogFileDir() {
        // 创建log.csv，若存在则删除
        deleteLogFile();
        File dir = new File(Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DOCUMENTS).getPath() + File.separator + LOG_DIR_NAME);
        if (dir.exists()) {
    ...
    private File getLogFile() {
        ...
        // 判断是否有SD卡或者外部存储器
        try {
            if (useSdCard && Environment.getExternalStorageState().equals(Environment.MEDIA_MOUNTED) && Build.VERSION.SDK_INT < Build.VERSION_CODES.P + 1) {
                // 有SD卡则使用SD - PS:没SD卡但是有外部存储器，会使用外部存储器
                // SD\OSSLog\logs.csv
                canStorage = readSDCardSpace() > LOG_MAX_SIZE / 1024;
                file = new File(Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DOCUMENTS).getPath() + File.separator + LOG_DIR_NAME);
            } else {
